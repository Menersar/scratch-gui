...
import {base64ToArrayBuffer} from './sidekick-base64-utils';

// // !!! ???
// // Special constants -- do not change without care.
// const DATABASE_NAME = 'Sidekick_AutoSave';
...
const DATABASE_NAME = 'Sidekick_RestorePoints';
...
const deleteLegacyRestorePoint = () => {
    const LEGACY_DATABASE_NAME = 'Sidekick_AutoSave';
    try {
        if (typeof indexedDB !== 'undefined') {
...
const INTERVAL_STORAGE_KEY = 'sidekick:restore-point-interval';
...
const readInterval = () => {
    try {
        ...
        const addonSettings = localStorage.getItem('sidekick:addons');
        if (addonSettings) {
            const parsedAddonSettings = JSON.parse(addonSettings);
            const addonObject = parsedAddonSettings['sidekick-disable-restore-points'];
